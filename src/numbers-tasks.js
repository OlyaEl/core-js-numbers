...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const point1 = x2 - x1;
  const point2 = y2 - y1;
  const result = Math.sqrt(point1 * point1 + point2 * point2);
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorXmodulus = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorYmodulus = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalarProduct / (vectorXmodulus * vectorYmodulus));
}
